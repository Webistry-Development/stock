...
export function automateFromMetadata(metafields) {
  metafields.forEach((metafield) => {
    const elem = document.head.querySelector(`meta[name='${metafield}']`);

    if (elem) {
      let { content } = elem;

      if (content.endsWith('.png')) {
        content = createOptimizedPicture(content).outerHTML;
      }

      const regexp = new RegExp(`\\[\\[${metafield}\\]\\]`, 'g');
      document.body.innerHTML = document.body.innerHTML.replace(regexp, content);
    }
  });
}
